...
public class ServiceDescriptionUpdater {
	...
	@Value( "${domain-name:localhost:8888}" )
	...
	public void refreshSwaggerConfigurations() {
		...
		discoveryClient.getServices().stream().forEach(serviceId -> {
			...
			if (serviceInstances == null || serviceInstances.isEmpty()) { // Should not be the case kept for failsafe
				...
				logger.info("Get swagger from url : {} ", swaggerURL);
	...
	public String getJSON(String serviceId, Object jsonData) {
		try {
			...
			LinkedHashMap<String, Object> jsonHashMap = (LinkedHashMap<String, Object>) jsonData;
			jsonHashMap.put("basePath", "/" + serviceId);
			jsonHashMap.put("host", baseUrl);
